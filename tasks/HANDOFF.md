...
- **Bilingual active product implemented locally (2026-04-27)**: active public reader, studio/auth/onboarding/atlas, seed demo, REST API v1, OpenAPI, MCP, `llms.txt`, `llms-full.txt`, sitemap alternates, metadata/JSON-LD, docs, and continuity files now support English + Simplified Chinese. Locale precedence: explicit `/zh` or `/en` and `?lang=zh|en`, then `lc_locale`, then `Accept-Language`, then English. Stable identifiers stay English-compatible. Migration `0004_i18n_translations.sql` adds `translations jsonb` to projects/stops/postcards/assets; seed Chinese lives under `translations.zh`; production migration/seed resync still requires a rollout step after deploy.
