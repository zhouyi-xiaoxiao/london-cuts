...
- **M7/M8 AI-native discovery/API/MCP live (2026-04-26/27)**: canonical public DTO seam `web/lib/public-content.ts`; public REST API v1 at `/api/v1/projects`; OpenAPI at `/api/openapi.json`; MCP JSON-RPC endpoint at `/mcp`; discovery files `/llms.txt`, `/llms-full.txt`, `/robots.txt`, `/sitemap.xml`; metadata + JSON-LD for public project/chapter/postcard pages. M8 adds richer DTO fields, structured markdown citation packs, `GET /api/v1/projects/{handle}/{slug}/ai-visibility`, MCP tool `audit_public_project_visibility`, and token issuance script `web/scripts/issue-agent-token.mjs`. Production smoke passed. `0003_api_tokens.sql` is applied, and owner full-scope token label `owner-full-agent-20260427` is stored in macOS Keychain service `london-cuts-agent-token`.
- **Bilingual active product deployed (2026-04-27)**: active public reader, studio/auth/onboarding/atlas, seed demo, REST API v1, OpenAPI, MCP, `llms.txt`, `llms-full.txt`, sitemap alternates, metadata/JSON-LD, docs, and continuity files support English + Simplified Chinese. Locale precedence: explicit `/zh` or `/en` and `?lang=zh|en`, then `lc_locale`, then `Accept-Language`, then English. Stable identifiers stay English-compatible. Migration `0004_i18n_translations.sql` is applied in production; seed demo rows now carry `translations.zh` for projects/stops/postcards/assets; production smoke passed for `/zh`, `/en`, API v1, OpenAPI, MCP, llms, sitemap alternates, and DB fallback/base-field behavior. Next track is M4 public polish. Authenticated Studio bilingual smoke is still worth doing via a normal email sign-in; do not use admin-generated callback URLs in browser tools that echo final URLs.
...
- Responsive QA status (2026-04-25T00:21Z): public project/chapter/postcard/sign-in/studio-redirect were swept at 1440, 768, 390, 360, and 320px with local Chrome + playwright-core. Fixes landed for mobile gutters, chapter title wrapping/sizing, public/export/mode/flip tap targets, and postcard flip caption wrapping. `web/tests/responsive.test.tsx` guards the mobile Workspace stop switcher and postcard flip touch target. Full `pnpm lint` still has unrelated pre-existing lint debt; touched-file ESLint is clean.
- Full-web QA status (2026-04-25T23:10Z): full lint debt is now cleared. Verification run: `pnpm lint`, `pnpm typecheck`, `pnpm test` (70/70), `pnpm build`, production route/API smoke, production public Chrome responsive sweep, valid magic-link send + Gmail receipt check, and authenticated Studio Chrome sweep. Found and fixed one real issue: authenticated `/studio` dashboard emitted React hydration error #418 because seed project/archive timestamps used `new Date()` / `Date.now()` during SSR/client render. `web/stores/root.ts` now uses deterministic seed timestamps. Latest fix commit `3a21d15`; Vercel deployment `dpl_2EoRMubiASWJoDoacCMuYJtYYhUY` Ready and aliased to `https://london-cuts.vercel.app`.
...
- Bilingual reads: use `/zh/...`, `/en/...`, or `?lang=zh|en`. MCP accepts locale through query/header/cookie or `locale` / `outputLocale` arguments. OpenAPI `?lang=zh` localizes descriptions only; operationIds/schema keys/scopes/tool names stay stable.
...
- Owner token: label `owner-full-agent-20260427`, scopes `public:read,ai:run,project:write`, plaintext stored only in macOS Keychain service `london-cuts-agent-token`.

Operational notes:
- Do not print, copy into docs, or commit the owner token. Retrieve it only into local shell variables for controlled smoke/agent runs.
- Temporary read-only negative-scope token `tmp-readonly-negative-20260427` was revoked and removed from Keychain after smoke.
...
- Token smoke passed: full token reached MCP `describe_photo`/`sync_project` auth and scope checks and stopped on missing input, so no AI spend or project write occurred. Read-only token failed `ai:run` as expected.
