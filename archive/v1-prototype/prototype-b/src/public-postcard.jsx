        ...
        <div className="row between items-baseline" style={{ marginBottom: 24 }}>
          ...
          <div className="row gap-12">
            ...
            <button className="btn btn-solid" onClick={() => navigator.share
              ...
              : navigator.clipboard?.copyText(location.href).then(() => alert('Link copied.'))
            }>Share</button>
          </div>
        </div>
...
function PostcardBack({ mode }) {
  ...
  const state = useStore(s => {
    ...
    return {
      message: stop?.message || DEFAULT_MESSAGE,
      recipient: stop?.recipient || DEFAULT_RECIPIENT,
    };
  });

  const [editingMessage, setEditingMessage] = React.useState(false);
  const [editingRecipient, setEditingRecipient] = React.useState(false);
  const [msgBuf, setMsgBuf] = React.useState(state.message);
  const [rcptBuf, setRcptBuf] = React.useState({ ...state.recipient });
  const [saveMsg, setSaveMsg] = React.useState('');

  // Sync buffer when store changes (external update)
  React.useEffect(() => {
    if (\!editingMessage) setMsgBuf(state.message);
    if (\!editingRecipient) setRcptBuf({ ...state.recipient });
  }, [state.message, state.recipient, editingMessage, editingRecipient]);

  const saveMessage = React.useCallback(() => {
    const trimmed = msgBuf.trim();
    if (trimmed) {
      storeActions.setStopMessage(stopId, trimmed);
      setSaveMsg('·saved');
  ...
  const saveRecipient = React.useCallback(() => {
    const addressLines = rcptBuf.addressLines.filter(l => l.trim()).slice(0, 4);
    if (addressLines.length === 0) addressLines.push('');
    const safe = {
      name: rcptBuf.name.trim() || 'Recipient',
      addressLines,
      country: rcptBuf.country.trim() || 'Country',
    };
    storeActions.setStopRecipient(stopId, safe);
    setSaveMsg('·saved');
    setTimeout(() => setSaveMsg(''), 1000);
    setEditingRecipient(false);
  }, [rcptBuf]);

  const handleKeyMessage = React.useCallback((e) => {
    if (e.key === 'Escape') {
      setMsgBuf(state.message);
      setEditingMessage(false);
  ...
  const handleKeyRecipient = React.useCallback((e) => {
    if (e.key === 'Escape') {
      setRcptBuf({ ...state.recipient });
      setEditingRecipient(false);
    } else if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
      saveRecipient();
    }
  }, [state.recipient, saveRecipient]);
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: 'oklch(0.96 0.008 60)', color: 'oklch(0.15 0.008 60)', padding: 32, display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 24, fontFamily: 'var(--f-mono)' }}>
      <div style={{ borderRight: '1px solid oklch(0.8 0.008 60)', paddingRight: 24 }} className="pc-message-section" data-editable={editingMessage}>
        {editingMessage ? (
          <textarea
            className={`pc-message-edit pc-edit-${mode}`}
            value={msgBuf}
            onChange={(e) => setMsgBuf(e.target.value)}
            onBlur={saveMessage}
            ...
            style={{
              ...
              resize: 'none',
              outline: 'none',
              borderBottom: '1px solid oklch(0.55 0.14 20)',
            }}
          />
        ) : (
          <div
            className="pc-message-view"
          ...
          >
            ...
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, pointerEvents: 'none', opacity: 0, transition: 'opacity 0.2s' }} className="pc-message-hover" />
          </div>
        )}
        {saveMsg && \!editingMessage && (
          <div className="pc-save-indicator" style={{ fontFamily: 'var(--f-mono)', fontSize: 11, marginTop: 8, opacity: 0.5, animation: 'fadeOut 1s' }}>
            {saveMsg}
          </div>
        )}
      </div>
      <div className="col between pc-recipient-section" data-editable={editingRecipient}>
        <div>
          <div className="mono-sm" style={{ letterSpacing: '0.25em', marginBottom: 12 }}>LONDON CUTS · ED.01 / 05</div>
          {editingRecipient ? (
            <div className="pc-recipient-edit col gap-0">
              <input
                type="text"
                placeholder="Name"
                value={rcptBuf.name}
                onChange={(e) => setRcptBuf({ ...rcptBuf, name: e.target.value })}
                onBlur={saveRecipient}
                onKeyDown={handleKeyRecipient}
                autoFocus
                style={{
              ...
              />
              {rcptBuf.addressLines.map((line, idx) => (
                <input
                  key={idx}
                  type="text"
                  ...
                  value={line}
                  onChange={(e) => {
                    const updated = rcptBuf.addressLines.slice();
                    updated[idx] = e.target.value;
                    setRcptBuf({ ...rcptBuf, addressLines: updated });
                  }}
                  onBlur={saveRecipient}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && idx === rcptBuf.addressLines.length - 1 && rcptBuf.addressLines.length < 4) {
                      const updated = rcptBuf.addressLines.slice();
                      updated.push('');
                      setRcptBuf({ ...rcptBuf, addressLines: updated });
                    } else if (e.key === 'Backspace' && line === '' && rcptBuf.addressLines.length > 1) {
                      const updated = rcptBuf.addressLines.filter((_, i) => i \!== idx);
                      setRcptBuf({ ...rcptBuf, addressLines: updated });
                    } else {
                      handleKeyRecipient(e);
                    }
                  }}
                  style={{
                    fontFamily: 'var(--f-mono)',
              ...
              <input
                type="text"
                placeholder="Country"
                value={rcptBuf.country}
                onChange={(e) => setRcptBuf({ ...rcptBuf, country: e.target.value })}
                onBlur={saveRecipient}
                onKeyDown={handleKeyRecipient}
                style={{
                  fontFamily: 'var(--f-mono)',
            ...
            <div
              className="pc-recipient-view"
              onClick={() => setEditingRecipient(true)}
              style={{ cursor: 'pointer', position: 'relative' }}
            >
              <div style={{ borderBottom: '1px solid oklch(0.4 0.008 60)', padding: '4px 0', fontSize: 12 }}>
                {state.recipient.name}
              </div>
              {state.recipient.addressLines.map((line, i) => (
                <div key={i} style={{ borderBottom: '1px solid oklch(0.4 0.008 60)', padding: '4px 0', fontSize: 12 }}>
                  {line}
                </div>
              ))}
              <div style={{ borderBottom: '1px solid oklch(0.4 0.008 60)', padding: '4px 0', fontSize: 12 }}>
              ...
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, pointerEvents: 'none', opacity: 0, transition: 'opacity 0.2s' }} className="pc-recipient-hover" />
            </div>
          )}
          {saveMsg && \!editingRecipient && (
            <div className="pc-save-indicator" style={{ fontFamily: 'var(--f-mono)', fontSize: 11, marginTop: 8, opacity: 0.5, animation: 'fadeOut 1s' }}>
              {saveMsg}
            </div>
          )}
