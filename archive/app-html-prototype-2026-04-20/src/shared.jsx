...
function Img({ label, tone, ratio = '3/2', style = {}, className = '', src, onError }) {
  ...
  if (src && !broken) {
    return (
      <img
        ...
        alt={label || ''}
        className={className}
        onError={(e) => { setBroken(true); onError && onError(e); }}
        style={{ aspectRatio: ratio, width: '100%', objectFit: 'cover', display: 'block', ...style }}
      />
    );
  }
  return (
